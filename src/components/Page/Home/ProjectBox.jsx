...
const ProjectBox = ({ data }) => {
  ...
  const fetchUser = async () => {
    try {
      const response = await axios.get(
        process.env.REACT_APP_API_URL + `/user/${data.publisher}`
      );
  ...
  const status =
    data.status === "모집중" ? "bg-white" : "bg-gray-200 text-white";
    ...
    >
      <div className="flex flex-col gap-2 w-full">
        ...
        >
          ...
          <hr />
          <div className="font-semibold text-lg p-3 truncate">{data.name}</div>
          ...
          <div className="flex flex-wrap flex-row gap-2 font-semibold text-neutral-500 p-3">
            {data.wanted.map((tmp) => (
              <div className="px-3 py-1 text-xs bg-gray-200 rounded-full text-gray-700">
                {tmp.stack}
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-wrap flex-row gap-2 font-semibold text-neutral-500 p-3">
            {data.stack.map((tmp) => (
              <div className="px-3 py-1 text-xs bg-gray-100 rounded-full text-sky-800">
                {tmp}
              </div>
            ))}
          </div>
          <div className="flex justify-start m-3">
            <div
              className={`text-sm px-6 py-1 my-2 rounded-lg font-semibold border-2 ${status}`}
            >
              {data.status}
            </div>
